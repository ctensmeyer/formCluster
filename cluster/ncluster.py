...
def overall(docs, num_subset, num_seeds, num_initial_cluster_range, min_pts):
	...
	for num_initial_clusters in xrange(*num_initial_cluster_range):
		...
		for name, clusters in {"init": initial_clusters, "split": sclusters, "final": final_clusters}.iteritems():
			...
			print "%s\t%d\t%d\t%.4f\t%.4f" % (name, k, init_k, acc, v)
